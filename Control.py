...
class MyExperiment:
    def __init__(self, project_name='My experient'):
        self.project_name = project_name

    def initDyn(self, alpha = None, beta1 = None, beta2 = None, Ib = None, gamma = None, S = None, delta = None):
        parameter = []

        # If a variable is none, it will be pushed to aux variable and train to get the best result
        ...
        if beta1 is None:
            self.beta1 = SX.sym('beta1')
            parameter += [self.beta1]
        else:
            self.beta1 = beta1
        
        if beta2 is None:
            self.beta2 = SX.sym('beta2')
            parameter += [self.beta2]
        else:
            self.beta2 = beta2
        ...
        if delta is None:
            self.delta = SX.sym('Ib')
            parameter += [self.delta]
        else:
            self.delta = delta
        
        self.dyn_auxvar = vcat(parameter)
        # set variable
        self.G1, self.G2, self.I = SX.sym('G1'), SX.sym('G2'), SX.sym('I')
        self.X = vertcat(self.G1, self.G2, self.I)
        self.U = SX.sym('u')
        # A vector denote the parameteres of the problem (G1, G2, I) tranposed

        dG1 = -self.alpha * self.G1
        dG2 = self.alpha * self.G1 - (self.beta1 + self.beta2 * (self.I + self.Ib)) * self.G2
        dI = self.gamma * (self.G2 - self.S) - self.delta * self.I + self.U
        self.f = vertcat(dG1, dG2, dI)
    
    def initCost(self):
        self.path_cost = (self.G2 - 50) * (self.G2 - 100) + self.U
        self.final_cost = (self.G2 - 50) * (self.G2 - 100)
...
experiment.initDyn(alpha = 2.3624595, beta1 = 0.047694835, beta2 = 0.029979771, Ib = 4, gamma = 0.8884565, S = 92.5, delta = 1.31002741)
experiment.initCost()
...
horizon = 300
ini_state = [100, 100, 5]
dyn = experiment.X + dt * experiment.f

solver = OCSys()
solver.setStateVariable(experiment.X)
solver.setControlVariable(experiment.U, control_lb = [0.2], control_ub = [250.0])
solver.setDyn(dyn)
solver.setPathCost(experiment.path_cost)
solver.setFinalCost(experiment.final_cost)
true_sol = solver.ocSolver(ini_state=ini_state, horizon=horizon)

print(true_sol['cost'])
print(true_sol["state_traj_opt"][0][0])
G1, G2, I, U = [], [], [], []
for i in range(horizon):
    G1.append(true_sol["state_traj_opt"][i][0])
    G2.append(true_sol["state_traj_opt"][i][1])
    I.append(true_sol["state_traj_opt"][i][2])
    U.append(true_sol["control_traj_opt"][i][0])
...
plt.plot(time, G1, label = "G1")
plt.plot(time, G2, label = "G2", color = "red")
plt.plot(time, I, label = "I", color = "green")
plt.plot(time, U, label = "U", color = "orange")
plt.legend(loc='upper right')

plt.axhline(y=50, color='black', linestyle='--', label='Hypoglycemia threshold')
plt.axhline(y=110, color='gray', linestyle='-.', label='Diabetes threshold')
